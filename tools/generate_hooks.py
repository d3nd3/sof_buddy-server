#!/usr/bin/env python3
# Adapted from public sof_buddy (tools/generate_hooks.py). Output dir: SOF_BUDDY_SERVER_GEN_DIR or ./build.
...
def parse_features_yaml(yaml_path):
    """Load enabled feature names from features/features.yaml."""
    enabled = set()
    ...
    if isinstance(feat, dict):
        for name, on in feat.items():
            if on is True:
                enabled.add(str(name))
    lst = data.get('enabled')
...
def parse_feature_toggles(features_dir):
    """
    Prefer features/features.yaml (explicit booleans, comments, optional `enabled:` list).
    Fall back to legacy features/FEATURES.txt if the YAML file is missing.
    """
    features_dir = Path(features_dir)
    yaml_path = features_dir / 'features.yaml'
    txt_path = features_dir / 'FEATURES.txt'
    if yaml_path.is_file():
        return parse_features_yaml(yaml_path)
    if txt_path.is_file():
        return parse_features_txt(str(txt_path))
    return set()
                    ...
                    with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                        ...
                        for func_name in all_detours.keys():
                            if re.search(r'\bo' + func_name + r'\b', content):
                                used_detours.add(func_name)
                            elif re.search(r'\bSOF_EP_' + func_name + r'\b', content):
                                used_detours.add(func_name)
                except Exception as e:
...
def generate_detours_header(detours, header_path, cpp_path, override_hooks, enabled_features, pointer_only_set):
    ...
    with open(cpp_path, 'w') as f:
        ...
        if override_hooks:
            ...
            f.write('// Note: Override hooks with custom_detour: true are manually installed at runtime and not generated here\n')
        ...
        for feature_name, hooks_path in feature_hooks:
            try:
                ...
                for hook in hooks:
                    ...
                    is_override = hook.get('override', False)
                    is_custom_detour = hook.get('custom_detour', False)
                    
                    if func_name not in detours:
                        if is_custom_detour:
                            # Custom detour - manually installed at runtime, not in detours.yaml
                            continue
                        if not is_override:
...
def default_return_stmt(return_type):
    rt = return_type.strip()
    if rt == 'void':
        return ''
    if rt in ('bool', 'qboolean'):
        return '    return 0;\n'
    if '*' in rt:
...
def emit_engine_pointer_fallback_stub(f, func_name, func):
    return_type = func['return_type']
    conv = func['calling_convention']
    params = func.get('params', [])
    variadic = func.get('variadic', False)
    params_decl = format_params_decl(params, variadic)
    stub = f'EnginePointerFallback_{func_name}'
    f.write(f'static {return_type} {conv} {stub}({params_decl}) {{\n')
    if return_type == 'void':
        if variadic:
            if params:
                pname = params[0]['name']
                ...
                f.write('    va_list ap;\n')
                f.write(f'    va_start(ap, {pname});\n')
                f.write(f'    vsnprintf(buf, sizeof(buf), {pname}, ap);\n')
                f.write('    va_end(ap);\n')
                f.write(f'    PrintOut(PRINT_LOG, "[o{func_name} fallback] %s", buf);\n')
            else:
                f.write('    (void)0;\n')
        else:
            for p in params:
                f.write(f'    (void){p["name"]};\n')
    else:
        if variadic and params:
            pname = params[0]['name']
            f.write(f'    (void){pname};\n')
            f.write('    va_list ap;\n')
            f.write(f'    va_start(ap, {pname});\n')
            f.write('    va_end(ap);\n')
        elif not variadic:
            for p in params:
                f.write(f'    (void){p["name"]};\n')
        dr = default_return_stmt(return_type)
        if dr:
            f.write(dr)
    f.write('}\n\n')


def generate_engine_pointers(pointer_only_set, detours, header_path, cpp_path):
    """
    Emit collision-safe call macros: SOF_EP_Name(...) -> detour_Name::oName(__VA_ARGS__).

    Do not use bare #define oName(...) — it breaks valid qualified calls like
    detour_Name::oName(...) (macro expands oName(...) and re-enters recursively).
    """
    names = sorted(n for n in pointer_only_set if n in detours)

    with open(header_path, 'w') as f:
        f.write('#pragma once\n\n')
        f.write('// Auto-generated — pointer-only symbols from pointers.json.\n')
        f.write('// Call via SOF_EP_<FunctionName>(...) — safe with detour_*::o* qualified names.\n')
        f.write('// tools/generate_hooks.py — do not edit.\n\n')
        f.write('#include "generated_detours.h"\n\n')
        for func_name in names:
            ns = f'detour_{func_name}'
            f.write(f'#define SOF_EP_{func_name}(...) {ns}::o{func_name}(__VA_ARGS__)\n')
        f.write('\nvoid EnginePointers_Bind();\n')

    with open(cpp_path, 'w') as f:
        f.write('// Auto-generated — EnginePointers_Bind() for pointer-only symbols.\n')
        f.write('// tools/generate_hooks.py — do not edit.\n\n')
        f.write('#include "generated_engine_pointers.h"\n')
        if names:
            f.write('#include "log.h"\n\n')
            f.write('#include <cstdarg>\n')
            f.write('#include <cstdio>\n\n')
        for func_name in names:
            emit_engine_pointer_fallback_stub(f, func_name, detours[func_name])
        f.write('void EnginePointers_Bind() {\n')
        for func_name in names:
            stub = f'EnginePointerFallback_{func_name}'
            ns = f'detour_{func_name}'
            f.write(f'    if (!{ns}::o{func_name}) {ns}::o{func_name} = {stub};\n')
        f.write('}\n')
...
def main():
    ...
    # --list-enabled: print the enabled feature names, one per line, and exit.
    # CMake calls this at configure time so it compiles exactly the features
    # this file turns on. Sharing the parser is the point - a second
    # implementation of features.yaml in CMake would drift, and the failure
    # mode of that drift is a disabled feature still being compiled against
    # detour namespaces that were deliberately not generated.
    if '--list-enabled' in sys.argv:
        for name in sorted(parse_feature_toggles(project_root / 'src' / 'features')):
            print(name)
        return
    
    yaml_path = project_root / 'detours.yaml'
    feature_toggles_dir = project_root / 'src' / 'features'
    features_dir = project_root / 'src' / 'features'
    core_dir = project_root / 'src' / 'core'
    src_dir = project_root / 'src'
    gen_env = os.environ.get("SOF_BUDDY_SERVER_GEN_DIR")
    build_dir = Path(gen_env) if gen_env else (project_root / "build")
    ...
    engine_ptr_h = build_dir / 'generated_engine_pointers.h'
    engine_ptr_cpp = build_dir / 'generated_engine_pointers.cpp'
    
    enabled_features = parse_feature_toggles(feature_toggles_dir)
    ...
    feature_hooks = [(name, path) for name, path in all_feature_hooks if name == 'core' or name in enabled_features]
    feature_callbacks = [(name, path) for name, path in all_feature_callbacks if name == 'core' or name in enabled_features]
    all_feature_pointers = find_pointers_json_files(features_dir, core_dir)
    feature_pointers = [(name, path) for name, path in all_feature_pointers if name == 'core' or name in enabled_features]
    ...
    generate_detours_header(
        detours, detours_output, detours_cpp_output, override_hooks, enabled_features, pointer_only_set)
    generate_registrations_header(detours, feature_hooks, feature_callbacks, registrations_output, override_hooks)
    generate_engine_pointers(pointer_only_set, detours, engine_ptr_h, engine_ptr_cpp)

    print(f"Generated {detours_output}")
    print(f"Generated {detours_cpp_output}")
    print(f"Generated {registrations_output}")
    print(f"Generated {engine_ptr_h}")
    print(f"Generated {engine_ptr_cpp}")
