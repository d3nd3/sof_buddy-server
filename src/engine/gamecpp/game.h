...
typedef struct
{
	...
	void	(*positioned_sound) (vec3_t origin, edict_t *ent, int channel, int soundinedex, float volume, float attenuation, float timeofs, int localize);
	...
	int			(*ReadFromSavegame)(unsigned long chid, void *address, int length, void **addressptr);
#if (!_FINAL_) && (_RAVEN_)
	...
	cvar_t	*(*cvar) (const char *var_name, const char *value, int flags, cvarcommand_t command);
	...
	int		(*FS_LoadFile) (char *name, void **buf, bool OverridePak);
